...
export interface WriteSarifArtifactOptions {
  bundleDirectory: string;
  relativePath: string;
  serializedSarif: string;
}

export interface WriteExternalSarifArtifactOptions {
  targetPath: string;
  serializedSarif: string;
}

export async function writeSarifArtifact(
  options: WriteSarifArtifactOptions,
): Promise<string> {
  const bundlePath = validateBundlePath(options.relativePath);
  const data = Buffer.from(options.serializedSarif, "utf8");
  if (!data.toString("utf8").endsWith("\n")) {
    throw new Error("SARIF payload must end with a trailing newline.");
  }
...
export async function writeExternalSarifArtifact(
  options: WriteExternalSarifArtifactOptions,
): Promise<string> {
  ...
  try {
    ...
    if (parentStatus.isSymbolicLink()) {
      throw new Error(`Refusing to write SARIF through a symbolic link parent: ${parent}`);
    }
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
      throw error;
  ...
  try {
    ...
    if (targetStatus.isSymbolicLink()) {
      throw new Error(`Refusing to overwrite a symbolic link SARIF target: ${targetPath}`);
    }
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
      throw error;
