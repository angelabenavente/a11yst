...
export function resolveNextCatalogOptions(
...
function assertPositiveInteger(value: number, label: string): number {
  if (!Number.isFinite(value) || !Number.isInteger(value) || value <= 0) {
    throw new NextSourceValidationError(
...
export function normalizeNextRoutePath(route: string): string | undefined {
...
export function isNextScopeFile(frameworks: string[] | undefined): boolean {
  return frameworks?.includes("next") ?? false;
}
