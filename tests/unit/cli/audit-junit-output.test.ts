...
describe("formatAuditHuman JUnit output", () => {
  it("prints bundle JUnit path by default", () => {
    ...
    expect(output).toContain("JUnit report: reports/a11yst.junit.xml");
  ...
  it("prints custom and bundle paths when both exist", () => {
    const output = formatAuditHuman(
      ...
      { junitExternalPath: "./artifacts/a11yst.junit.xml" },
    );
    expect(output).toContain("JUnit report: ./artifacts/a11yst.junit.xml");
    expect(output).toContain("Bundle copy: reports/a11yst.junit.xml");
  });
});
