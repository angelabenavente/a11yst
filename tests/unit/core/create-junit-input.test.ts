...
import { createJunitInputFromAuditResult } from "@a11yst/core";
...
describe("createJunitInputFromAuditResult", () => {
  it("maps findings, policy, baseline, and product metadata", () => {
    ...
    const input = createJunitInputFromAuditResult(result);
    ...
    expect(input.audit.durationMs).toBe(1);
  });

  it("accepts legacy results without optional metadata", () => {
    const input = createJunitInputFromAuditResult(baseResult());
  ...
  it("does not mutate the source result", () => {
    ...
    createJunitInputFromAuditResult(result);
